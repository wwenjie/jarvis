...
def setup_logging():
    # 创建logs目录
    os.makedirs('/app/logs/agent', exist_ok=True)
    ...
    # 创建不同级别的文件处理器
    debug_handler = logging.FileHandler('/app/logs/agent/debug.log', encoding='utf-8')
    debug_handler.setLevel(logging.DEBUG)
    debug_handler.setFormatter(logging.Formatter(log_format))
    
    info_handler = logging.FileHandler('/app/logs/agent/info.log', encoding='utf-8')
    info_handler.setLevel(logging.INFO)
    info_handler.setFormatter(logging.Formatter(log_format))
    
    warning_handler = logging.FileHandler('/app/logs/agent/warning.log', encoding='utf-8')
    warning_handler.setLevel(logging.WARNING)
    warning_handler.setFormatter(logging.Formatter(log_format))
    
    error_handler = logging.FileHandler('/app/logs/agent/error.log', encoding='utf-8')
...
# API 网关配置
API_SERVICE_BASE_URL = "http://localhost:8081"  # 根据实际部署情况修改
...
async def upload_document(file: UploadFile = File(...)):
    ...
    # 调用 API 网关添加文档
    async with httpx.AsyncClient(base_url=API_SERVICE_BASE_URL, timeout=60.0) as client:
...
async def list_documents():
    try:
        logger.info(f"正在请求 API 网关: {API_SERVICE_BASE_URL}/document/list")
        logger.info(f"请求参数: user_id=1, page=1, page_size=100")
        async with httpx.AsyncClient(base_url=API_SERVICE_BASE_URL, timeout=5.0) as client:
            ...
            logger.info(f"API 网关响应状态码: {response.status_code}")
            logger.debug(f"API 网关响应内容: {response.text}")
...
async def process_web_search(arguments: dict):
    logger.info(f"开始处理网页搜索: {arguments}")
    try:
        import requests
...
async def process_function_call(func_call: dict, api_service_base_url: str) -> dict:
    """处理单个函数调用
    
    Args:
        func_call: 函数调用信息，包含name和arguments
        api_service_base_url: 后端API服务基础URL
    ...
    try:
        if func_call["name"] == "web_search":
            ...
            async with httpx.AsyncClient(base_url=api_service_base_url, timeout=10.0) as client:
                response = await client.get(
                    "/weather/get",
                    params={
            ...
            async with httpx.AsyncClient(base_url=api_service_base_url, timeout=20.0) as client:
                response = await client.get(
                    "/document/search",
                    params={
                        "user_id": 1,  # 固定用户ID
            ...
            async with httpx.AsyncClient(base_url=api_service_base_url, timeout=5.0) as client:
                ...
                logger.debug(f"发送添加记忆请求: {request_data}")  # 添加日志
                response = await client.post(
                    "/memory/add",
                    json=request_data
                )
                logger.debug(f"添加记忆响应: {response.status_code} - {response.text}")  # 添加日志
                if response.status_code == 200:
                    result = response.json()
                    # 兼容不同的响应格式
            ...
            async with httpx.AsyncClient(base_url=api_service_base_url, timeout=5.0) as client:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        logger.error("解析响应失败:", e, response.text)
                        return {
                            "name": "delete_memory",
                            "result": "error",
                    ...
                    logger.warning(f"删除记忆响应: {response.text}")
                    return {
                        "name": "delete_memory",
                        "result": "error",
                        "error": f"请求失败: {response.status_code}"
...
async def process_stream_request(query: str, session_id: str = None, web_search: bool = False):
    logger.info(f"开始处理流式请求: query={query}, session_id={session_id}")
    ...
    if not session_id:
        ...
        async with httpx.AsyncClient(base_url=API_SERVICE_BASE_URL, timeout=5.0) as client:
            ...
            logger.info(f"新会话创建成功: session_id={session_id}")

    # # 检索相关文档
    # logger.info("开始检索相关文档...")
    # try:
    #     async with httpx.AsyncClient(base_url=API_SERVICE_BASE_URL, timeout=30.0) as client:
    #         logger.info(f"请求 API 网关: {API_SERVICE_BASE_URL}/document/search")
    #         logger.info(f"请求参数: user_id=1, query={query}, top_k=3")
    ...
    #     logger.error(f"请求 API 网关失败: {str(e)}")
        ...
        try:
            async with httpx.AsyncClient(base_url=API_SERVICE_BASE_URL, timeout=5.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    logger.warning(f"获取历史消息失败: {resp.status_code} {resp.json()}")
        except Exception as e:
...
"""
    ...
    while True:
        ...
        if response.choices[0].message.function_call:
            logger.debug(f"Function Call: {response.choices[0].message.function_call.name}")
            logger.debug(f"Arguments: {response.choices[0].message.function_call.arguments}")
        logger.debug("==================\n")
        ...
        if response.choices[0].message.function_call:
            ...
            try:
                ...
                logger.error(f"解析函数调用参数失败: {str(e)}")
                continue
        
        # 处理函数调用结果
        if function_calls:
            logger.info(f"处理函数调用结果: {function_calls}")
            function_results = []
            
            # 处理每个函数调用
            for func_call in function_calls:
                result = await process_function_call(func_call, API_SERVICE_BASE_URL)
                function_results.append(result)
            
            # 将函数调用结果添加到对话历史中
            if function_results:
                logger.debug(f"函数调用结果: {function_results}")
                # 添加AI的函数调用请求
                messages.append({
                    "role": "assistant",
                    "content": None,
    ...
    if session_id and full_response:
        try:
            async with httpx.AsyncClient(base_url=API_SERVICE_BASE_URL, timeout=5.0) as client:
                ...
                if response.status_code != 200:
                    logger.error(f"保存聊天记录失败: HTTP {response.status_code}")
                else:
                    result = response.json()
                    if result.get("code", 0) != 0:
                        logger.error(f"保存聊天记录失败: {result.get('msg', '未知错误')}")
                    else:
                        logger.info(f"聊天记录保存成功: chat_id={result.get('chat_id')}")
        except Exception as e:
            logger.error(f"保存聊天记录时发生错误: {str(e)}")
...
async def get_chat_history():
    try:
        logger.info("开始获取聊天历史...")
        async with httpx.AsyncClient(base_url=API_SERVICE_BASE_URL, timeout=5.0) as client:
            logger.info(f"请求 API 网关: {API_SERVICE_BASE_URL}/session/list")
            response = await client.get(
                "/session/list",
                params={
                    "user_id": 1,  # 这里需要根据实际用户ID修改
            ...
            logger.info(f"成功获取到 {len(sessions)} 个会话")
            return sessions
    except Exception as e:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if result.get("success") and result.get("results"):
                # 将所有识别到的文本和位置信息合并，发送给翻译API
                ocr_text_with_bbox = []
                for i, item in enumerate(result["results"], 1):
                    text = item.get("text", "")
                    bbox = item.get("bbox", [])
                    confidence = item.get("confidence", 0)
                    ocr_text_with_bbox.append(f"'text{i}': '{text}', 'bbox': {bbox}, 'confidence': {confidence:.3f}")
                
                combined_text = "\n".join(ocr_text_with_bbox)
                ...
                if combined_text:
                    try:
                        ...
                        # 更新结果，添加预测信息
                        result["predictions"] = translation_result
                    except Exception as e:
    ...
    try:
        ...
        response = openai_client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": """你是一个专业的翻译助手。请分析OCR识别的文本片段，判断哪些片段能组成完整句子，补充漏掉的单词，然后翻译完整句子。

任务要求：
1. 首先判断文本是否为英文：非英文文本不翻译，直接跳过，只对英文文本进行翻译
2. 分析文本片段，判断哪些片段能组成完整的英文句子，将能组成完整句子的片段组合起来，比如下面示例中，5行文本的位置都在同一块区域，而且语义明显能组成一个句子
3. 补充OCR识别时漏掉的单词，比如下面输入实例中，"you dont know that"和"love YoU。"中漏了"I"，这是ocr识别时漏掉的单词，需要补充完整
4. 翻译完整的句子，而不是每个片段，比如下面输入实例中，5行文本的位置都在同一块区域，而且语义明显能组成一个句子，需要翻译完整的句子，而不是每个片段，如果文本片段不能组成完整句子，而且明显不在图片同一块区域，则单独翻译每个片段
5. 如果文本片段不能组成完整句子，而且明显不在图片同一块区域，则单独翻译每个片段
6. 注意置信度信息：置信度越高表示OCR识别越准确，置信度低的文本可能存在识别错误，需要根据上下文进行修正

返回格式：
- 只返回英文文本的翻译
- 每行一个完整句子的翻译
- 格式：完整英文句子 -> 中文翻译
- 如果某个片段不能组成完整句子，则单独翻译
- 如果多个片段都无法组成完整句子，则每个片段单独翻译，格式：原文 -> 中文翻译
- 非英文文本不返回

例如：
输入：
'text1': 'The furthest distance in the world is', 'bbox': [[36.0, 20.0], [494.0, 20.0], [494.0, 52.0], [36.0, 52.0]], 'confidence': 0.669
'text2': 'not between life and death。', 'bbox': [[36.0, 68.0], [384.0, 68.0], [384.0, 100.0], [36.0, 100.0]], 'confidence': 0.767
'text3': 'But When | stand in front of you;yet', 'bbox': [[35.0, 117.0], [485.0, 117.0], [485.0, 156.0], [35.0, 156.0]], 'confidence': 0.512
'text4': 'you dont know that', 'bbox': [[35.0, 165.0], [297.0, 165.0], [297.0, 203.0], [35.0, 203.0]], 'confidence': 0.884
'text5': 'love YoU。', 'bbox': [[305.0, 167.0], [423.0, 167.0], [423.0, 205.0], [305.0, 205.0]], 'confidence': 0.355

输出：
The furthest distance in the world is not between life and death. But when I stand in front of you, yet you don't know that I love you. -> 世界上最远的距离不是生与死之间的距离。而是当我站在你面前时，你却不知道我爱你。

如果输入是分散的文本：
输入：
'text1': 'train', 'bbox': [[100, 100], [200, 100], [200, 150], [100, 150]], 'confidence': 0.985
'text2': '山巅酌风', 'bbox': [[300, 200], [400, 200], [400, 250], [300, 250]], 'confidence': 0.876
'text3': 'station', 'bbox': [[500, 300], [600, 300], [600, 350], [500, 350]], 'confidence': 0.765
...
- 只翻译英文文本，非英文文本不翻译
- 补充漏掉的单词（例如上面例子的"I"）
- 修正标点符号和大小写或识别错误的字符（比如上述例子"I"识别成了"|"）
- 确保语法正确
- 参考置信度信息：置信度低的文本可能存在识别错误，需要根据上下文和语法进行修正
- 如果文本片段无法组成有意义的句子，则每个片段单独翻译"""
                },
                {
                    "role": "user",
                    "content": f"""请分析并翻译以下OCR识别的文本片段：

{combined_text}

请按照以下步骤处理：
1. 首先判断每个文本是否为英文（包含中文字符、日文字符等非英文字符的文本不是英文）
2. 只对英文文本进行翻译，非英文文本（如中文、日文等）不翻译，直接跳过
3. 分析英文文本片段是否能组成完整的英文句子：
   - 如果能组成完整句子，请组合并翻译完整句子
   - 如果无法组成完整句子，请每个片段单独翻译
4. 注意修正OCR识别错误（如拼写错误、标点符号错误等）

请只返回英文文本的翻译结果，格式为"原文 -> 中文翻译"。"""
                }
            ],
            max_tokens=500,
            temperature=0.3
        ...
        for line in translation_lines:
            if "->" in line:
                ...
                if len(parts) == 2:
                    ...
                    predictions.append({
                        "predicted_text": complete_sentence,
