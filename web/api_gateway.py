...
async def list_documents():
    try:
        logger.info(f"正在请求 API 网关: {API_GATEWAY_BASE_URL}/document/list")
...
async def process_stream_request(query: str, session_id: str = None, web_search: bool = False):
    ...
    #         logger.info(f"请求 API 网关: {API_GATEWAY_BASE_URL}/document/search")
...
async def get_chat_history():
    try:
        ...
        async with httpx.AsyncClient(base_url=API_GATEWAY_BASE_URL, timeout=5.0) as client:
            logger.info(f"请求 API 网关: {API_GATEWAY_BASE_URL}/session/list")
            response = await client.get(
                "/session/list",
                params={
                    "user_id": 1,  # 这里需要根据实际用户ID修改
